...
# 
# # Federal Investment on Education
# df$Total_Federal_Investment <- df$Total_Federal_Investment / df$Total_Population
# 
# # Hospital beds
# df$COVID_Hospital_All_Beds <- df$COVID_Hospital_All_Beds / df$Total_Population
# 
# # Covid tests
# df$COVID_Average_TestCapacity <- df$COVID_Average_TestCapacity / df$Total_Population
# df$COVID_Average_Testing <- df$COVID_Average_Testing / df$Total_Population
...
# For example, number of beds for 1000 people, occupancy rates every xx people
# The same with COVID-19 related indicators. (number of test every xx people, etc)


# Income, Federal Investment, Hospital All Beds, Population Density, COVID Death, confirmed, average testing


myscaler <- function(column){
    scaled_column <- (column - min(column, na.rm = T)) / (max(column, na.rm = T) - min(column, na.rm = T))
  return(scaled_column)
}

df <- df %>%
          mutate_at(vars('Total_Federal_Investment', 'Median_Household_Income_2019', 
                   'Mask_Usage_Combined', 'COVID_Average_Confirmed', 
                   'COVID_Average_Death', 'Population_Density', 
                   'Unemployed_2019', 'Poverty_Estimates_Value', 
                   'Total_Population'), myscaler)
